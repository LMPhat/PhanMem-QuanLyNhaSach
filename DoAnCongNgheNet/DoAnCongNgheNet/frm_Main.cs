    ...
    public partial class frm_Main : MetroFramework.Forms.MetroForm
    {
        ...
        private void btnLuu_Sach_Click(object sender, EventArgs e)
        {
            if (txtTenSach_Sach.Text.Trim().Length > 0 && txtNamXB_Sach.Text.Trim().Length > 0 && txtGiaBan_Sach.Text.Trim().Length > 0 && txtNoiDung_Sach.Text.Trim().Length > 0)
            {
                try
                {
                    ...
                    cmd.Parameters.Add("@matl", SqlDbType.VarChar).Value = cboTheLoai_Sach.SelectedValue.ToString();
                    cmd.Parameters.Add("@matg", SqlDbType.VarChar).Value = cboTacGia_Sach.SelectedValue.ToString();
                    cmd.Parameters.Add("@manxb", SqlDbType.VarChar).Value = cboNXB_Sach.SelectedValue.ToString();
        ...
        private void btnLapPhieu_LapPN_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //gọi proc tính thành tiền trong chi tiết phiếu nhập
                SqlCommand cmd = new SqlCommand("exec Proc_CapNhatDonGia_PhieuNhap", conn.Connect);
                conn.openConnection();
                cmd.ExecuteNonQuery();
                conn.closeConnection();
                //gọi proc tính Tổng tiền trong Phiếu nhập
                SqlCommand cmd1 = new SqlCommand("exec Proc_CapNhatThanhTien_PhieuNhap", conn.Connect);
                conn.openConnection();
